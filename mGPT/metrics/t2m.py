...
class TM2TMetrics(Metric):
    def __init__(self,
        ...
        self.add_state("how2sign_DTW_MPJPE_PA_wholebody",
                       default=torch.tensor([0.0]),
                       dist_reduce_fx="sum")
        self.add_state("csl_DTW_MPJPE_PA_wholebody",
                       default=torch.tensor([0.0]),
                       dist_reduce_fx="sum")
        self.add_state("phoenix_DTW_MPJPE_PA_wholebody",
                       default=torch.tensor([0.0]),
                       dist_reduce_fx="sum")

        self.MR_metrics = ["how2sign_DTW_MPJPE_PA_lhand", "how2sign_DTW_MPJPE_PA_rhand", "how2sign_DTW_MPJPE_PA_body", "how2sign_DTW_MPJPE_PA_wholebody",
                           "csl_DTW_MPJPE_PA_lhand", "csl_DTW_MPJPE_PA_rhand", "csl_DTW_MPJPE_PA_body", "csl_DTW_MPJPE_PA_wholebody",
                           "phoenix_DTW_MPJPE_PA_lhand", "phoenix_DTW_MPJPE_PA_rhand", "phoenix_DTW_MPJPE_PA_body", "phoenix_DTW_MPJPE_PA_wholebody"]
    ...
    def update(self, 
        ...
        for i in range(len(lengths)):
            ...
            if split in ['val', 'test']:
                ...
                # Whole-body: concatenate upper body joints + lhand joints + rhand joints
                joint_idx_body = self.joint_part2idx['upper_body']
                joints_rst_wb = np.concatenate([joints_rst_cur[:, joint_idx_body], joint_out_lhand, joint_out_rhand], axis=1)
                joints_ref_wb = np.concatenate([joints_ref_cur[:, joint_idx_body], joint_gt_lhand, joint_gt_rhand], axis=1)
                dist_func_wb = partial(l2_dist_align, align_idx=0)
                value = dtw(joints_rst_wb, joints_ref_wb, dist_func_wb)[0]
                setattr(self, f"{data_src}_DTW_MPJPE_PA_wholebody", getattr(self, f"{data_src}_DTW_MPJPE_PA_wholebody") + value)
                self.name2scores[cur_name][f"{data_src}_DTW_MPJPE_PA_wholebody"] = value
