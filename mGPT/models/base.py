...
class BaseModel(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if loss is not None:
            ...
            self._log_train_step_to_wandb(loss, batch_idx)
        return loss

    def _log_train_step_to_wandb(self, loss, batch_idx):
        if batch_idx % 10 != 0 or not getattr(self.trainer, "is_global_zero", True):
            return
        ...
        payload = {
            "total/train_step": float(loss.detach().cpu()),
        ...
        for logger in loggers:
            ...
            if hasattr(experiment, "log"):
                try:
                    experiment.log(payload, step=int(self.global_step), commit=True)
                except Exception:
                    pass
                break
    ...
    def test_step(self, batch, batch_idx):
        ...
        try:
            save_dir = os.path.join(self.output_dir, f'{self.hparams.cfg.TEST.SPLIT}_rank_'+str((torch.distributed.get_rank() if torch.distributed.is_initialized() else 0)))
        except:
    ...
    def on_test_epoch_end(self):
        ...
        if 'lm' in self.hparams.stage:
            ...
            metrics = ["how2sign_DTW_MPJPE_PA_lhand", "how2sign_DTW_MPJPE_PA_rhand", "how2sign_DTW_MPJPE_PA_body", "how2sign_DTW_MPJPE_PA_wholebody",
            ...
            print('rank: ', (torch.distributed.get_rank() if torch.distributed.is_initialized() else 0), scores)
