...
def create_logger(cfg, phase='train'):
    ...
    time_str = os.environ.get("SOKE_RUN_TIME") or time.strftime('%Y-%m-%d-%H-%M-%S')
    ...
    if os.path.exists(final_output_dir) and not os.path.exists(cfg.TRAIN.RESUME) and not cfg.DEBUG and phase not in ['test', 'demo']:
        ...
        for item in file_list:
            if item.endswith('.log'):
                archive_dir = Path(str(final_output_dir) + '_' + cfg.TIME)
                if archive_dir.exists():
                    suffix = 1
                    while archive_dir.with_name(f"{archive_dir.name}_{suffix}").exists():
                        suffix += 1
                    archive_dir = archive_dir.with_name(f"{archive_dir.name}_{suffix}")
                os.rename(str(final_output_dir), str(archive_dir))
                break
