...
from NIAF.continuous_sign_field.guava_gap import apply_bounded_correction
from NIAF.continuous_sign_field.models.meta_implicit import MetaImplicitResidualField
from NIAF.continuous_sign_field.scripts.train_guava_self_only_meta import (
    TARGET_SOURCE,
...
def _identity_compact(frames):
    ...
    return torch.cat([rot6d, torch.zeros(frames, 10)], dim=-1).numpy().astype(np.float32)
...
def test_zeroed_residual_head_uses_a_separate_warmup_learning_rate():
    ...
    assert info["residual_head_parameters"] == sum(p.numel() for p in model.out.parameters())
...
def test_guava_only_selection_rejects_motion_unsafe_alpha():
    ...
    assert details["candidate_audits"]["alpha_1.00"][
        "safety_constraints_passed"
    ] is False
