#!/bin/bash
#SBATCH --job-name=csl_stage_c_protocol_v5_run_r6_calibration
...
LAUNCHER="$PROJECT_DIR/scripts/NIAF/calibrate_csl_daily_stage_c_generator_adaptation_protocol_v5_run_r6_sbatch.sh"
...
STAGE_C_CONFIG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_protocol_v5_run_r6.yaml"
STAGE_C_OFF_CONFIG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_protocol_v5_run_r6.yaml"
...
CALIBRATION_OUT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v5_run_r6"
...
if [[ -e "$CALIBRATION_OUT" || -L "$CALIBRATION_OUT" \
  ...
  echo "ERROR: protocol-v5/run-r6 forbids calibration resume after partial output" >&2
  exit 1
fi
if [[ -d "${LEASE}.history" \
      && -n "$(find "${LEASE}.history" -mindepth 1 -print -quit)" ]]; then
  echo "ERROR: protocol-v5/run-r6 calibration lease history forbids retry" >&2
  exit 1
fi
...
PY
  ...
  [[ "$ACTIVE_OWNER_JOB" == "$SLURM_JOB_ID" && ! -e "$LEASE/TERMINAL.json" ]] || {
    echo "ERROR: protocol-v5/run-r6 forbids replacing a calibration owner" >&2
    exit 1
  }
elif [[ -e "$LEASE" || -L "$LEASE" ]]; then
  echo "ERROR: protocol-v5/run-r6 calibration lease is malformed" >&2
  exit 1
elif [[ -d "$CONTROL_DIR/lease_attestations" \
        && -n "$(find "$CONTROL_DIR/lease_attestations" -mindepth 1 -print -quit)" ]]; then
  echo "ERROR: protocol-v5/run-r6 orphaned calibration claim forbids retry" >&2
  exit 1
fi
...
"$PYTHON_BIN" -m "$CALIBRATION_CONTROL_MODULE" acquire \
  ...
  --source_remote_ref "origin/$SOURCE_REMOTE_BRANCH" \
  --source_remote_head "$REMOTE_HEAD" \
  --source_file_profile stage_c_generator_adaptation_protocol_v5_run_r6 \
...
validate_relevance_calibration_source(
    ...
    expected_source_file_profile="stage_c_generator_adaptation_protocol_v5_run_r6",
)
