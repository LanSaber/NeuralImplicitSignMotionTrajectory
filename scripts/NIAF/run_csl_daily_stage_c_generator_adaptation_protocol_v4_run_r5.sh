...
EXPECTED_SOURCE_REMOTE_BRANCH="codex/csl-daily-centered-generator-stage-c-v4-run-r5"
...
MEMORY_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_protocol_v4_run_r5"
OFF_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_protocol_v4_run_r5"
...
DECISION_POLICY="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_decision_policy_v1.json"
RECOVERY_MANIFEST="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_recovery_evidence_v1.json"
RECOVERY_EVIDENCE_ROOT="/media/cvpr/haomian/NeuralImplicitSignMotionTrajectory"
INCIDENT_ARCHIVE="$RECOVERY_EVIDENCE_ROOT/experiments/NIAF/continuous_trajectory_field/csl_daily_stage_c_generator_adaptation_protocol_v3_run_r4_cpu.invalid_attempts/source_740400412386d60a8d6f56b0f871bc16dee755b0_cpu143593_dependents143594_143596/ARCHIVE.json"
STAGER="$PROJECT_DIR/scripts/NIAF/stage_sentence_memory_train_val_only_node.sh"
CALIBRATION_LAUNCHER="$PROJECT_DIR/scripts/NIAF/calibrate_csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_sbatch.sh"
...
CALIBRATION_DIR="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v4_run_r5"
...
PREREQUISITE_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_prerequisites/source_${SOURCE_GIT_HEAD,,}"
...
SMOKE_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v4_run_r5_smoke"
PILOT_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v4_run_r5_pilot"
...
for cfg, arm, mode, probability in (
    ...
    assert stage == {
        ...
        "active_stage_c": {
            "calibration_artifact_dir": "experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v4_run_r5",
...
assert normalized_generation(memory) == normalized_generation(r2_memory), (
    "protocol-v4 memory science differs from immutable protocol-v2/run-r3"
)
assert normalized_generation(off) == normalized_generation(r2_off), (
    "protocol-v4 matched-off science differs from immutable protocol-v2/run-r3"
)
...
# Protocol v4/run-r5 permits no new logical retry. Inspect the global mode
...
if [[ -e "$ATTEMPTS_ROOT" || -L "$ATTEMPTS_ROOT" ]]; then
  [[ -d "$ATTEMPTS_ROOT" && ! -L "$ATTEMPTS_ROOT" \
    ...
    echo "ERROR: protocol-v4/run-r5 attempt tree contains a nonregular entry" >&2
    exit 1
  }
fi
[[ "${#PREEXISTING_COMPLETIONS[@]}" -le 1 ]] || {
  echo "ERROR: protocol-v4/run-r5 has multiple completed executions" >&2
  exit 1
}
if [[ "${#PREEXISTING_COMPLETIONS[@]}" == "0" \
      && "${#PREEXISTING_ATTEMPT_ENTRIES[@]}" != "0" ]]; then
  echo "ERROR: protocol-v4/run-r5 forbids retry/resume after partial execution" >&2
  exit 1
fi
...
PY
  ...
  if [[ "$ACTIVE_OWNER_JOB" != "$SLURM_JOB_ID" \
    ...
    echo "ERROR: protocol-v4/run-r5 forbids replacing another logical job" >&2
    exit 1
  fi
  if [[ -e "$LEASE/TERMINAL.json" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
    echo "ERROR: protocol-v4/run-r5 terminal lease cannot create a new attempt" >&2
    exit 1
  fi
elif [[ -e "$LEASE" || -L "$LEASE" ]]; then
  echo "ERROR: protocol-v4/run-r5 active lease is malformed" >&2
  exit 1
elif [[ -e "$MODE_CONTROL_DIR" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v4/run-r5 prior control state forbids a new attempt" >&2
  exit 1
fi
if [[ -d "${LEASE}.history" \
      && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v4/run-r5 terminal lease history forbids retry" >&2
  exit 1
fi
...
# restart generation. The run-r5 precheck above forbids another job from
...
SOURCE_BINDING_FILES=(
  ...
  "$PROJECT_DIR/scripts/NIAF/run_csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5.sh"
)
...
revalidate_source_binding() {
  ...
  "$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-protocol-v4-runtime --policy_path "$DECISION_POLICY" --recovery_manifest "$RECOVERY_MANIFEST" --source_root "$PROJECT_DIR" --evidence_root "$RECOVERY_EVIDENCE_ROOT" >/dev/null
