...
MEMORY_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_protocol_v2_run_r3"
OFF_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_protocol_v2_run_r3"
...
R2_MEMORY_CFG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_pilot_run_r2.yaml"
R2_OFF_CFG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_pilot_run_r2.yaml"
...
DECISION_POLICY="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3_decision_policy_v1.json"
...
CALIBRATION_LAUNCHER="$PROJECT_DIR/scripts/NIAF/calibrate_csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3_sbatch.sh"
...
CALIBRATION_DIR="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v2_run_r3"
...
SMOKE_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v2_run_r3_smoke"
PILOT_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v2_run_r3_pilot"
...
EXPECTED_R2_MEMORY_CONFIG_SHA256="62a6b955594580377638ee1d7a2f47ae4e62ae1ba7258f1cbedd474cf998612d"
EXPECTED_R2_OFF_CONFIG_SHA256="dc3722d838c45cb4480552f1db224b9c63f1fdc04cc363219f4d01f625f3b7a4"
...
for required in "$PYTHON_BIN" "$MEMORY_CFG" "$OFF_CFG" \
  "$R2_MEMORY_CFG" "$R2_OFF_CFG" "$STAGE_B_CFG" \
...
require_sha256 "$R2_MEMORY_CFG" "$EXPECTED_R2_MEMORY_CONFIG_SHA256" "run-r2 memory config"
require_sha256 "$R2_OFF_CFG" "$EXPECTED_R2_OFF_CONFIG_SHA256" "run-r2 matched-off config"
...
for cfg, arm, mode, probability in (
    ...
    assert stage == {
        ...
        "active_stage_c": {
            "calibration_artifact_dir": "experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v2_run_r3",
...
assert normalized_generation(memory) == normalized_generation(r2_memory), (
    "protocol-v2 memory science differs from immutable run-r2"
)
assert normalized_generation(off) == normalized_generation(r2_off), (
    "protocol-v2 matched-off science differs from immutable run-r2"
)
...
# Protocol v2/run-r3 permits no new logical retry.  Inspect the global mode
...
if [[ -e "$ATTEMPTS_ROOT" || -L "$ATTEMPTS_ROOT" ]]; then
  [[ -d "$ATTEMPTS_ROOT" && ! -L "$ATTEMPTS_ROOT" \
    ...
    echo "ERROR: protocol-v2/run-r3 attempt tree contains a nonregular entry" >&2
    exit 1
  }
fi
[[ "${#PREEXISTING_COMPLETIONS[@]}" -le 1 ]] || {
  echo "ERROR: protocol-v2/run-r3 has multiple completed executions" >&2
  exit 1
}
if [[ "${#PREEXISTING_COMPLETIONS[@]}" == "0" \
      && "${#PREEXISTING_ATTEMPT_ENTRIES[@]}" != "0" ]]; then
  echo "ERROR: protocol-v2/run-r3 forbids retry/resume after partial execution" >&2
  exit 1
fi
...
PY
  ...
  if [[ "$ACTIVE_OWNER_JOB" != "$SLURM_JOB_ID" \
    ...
    echo "ERROR: protocol-v2/run-r3 forbids replacing another logical job" >&2
    exit 1
  fi
  if [[ -e "$LEASE/TERMINAL.json" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
    echo "ERROR: protocol-v2/run-r3 terminal lease cannot create a new attempt" >&2
    exit 1
  fi
elif [[ -e "$LEASE" || -L "$LEASE" ]]; then
  echo "ERROR: protocol-v2/run-r3 active lease is malformed" >&2
  exit 1
elif [[ -e "$MODE_CONTROL_DIR" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v2/run-r3 prior control state forbids a new attempt" >&2
  exit 1
fi
if [[ -d "${LEASE}.history" \
      && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v2/run-r3 terminal lease history forbids retry" >&2
  exit 1
fi
...
# restart generation.  The run-r3 precheck above forbids another job from
...
SOURCE_BINDING_FILES=(
  ...
  "$PROJECT_DIR/scripts/NIAF/run_csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3.sh"
)
EXPECTED_SOURCE_BINDING_FILE_COUNT=29
if [[ "$EXECUTION_MODE" == "pilot" ]]; then
