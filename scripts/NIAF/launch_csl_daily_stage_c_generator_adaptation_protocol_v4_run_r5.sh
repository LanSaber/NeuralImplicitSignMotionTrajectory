...
BATCH_SCRIPT="$PROJECT_DIR/scripts/NIAF/train_csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_sbatch.sh"
SMOKE_SCRIPT="$PROJECT_DIR/scripts/NIAF/smoke_csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_sbatch.sh"
CALIBRATION_SCRIPT="$PROJECT_DIR/scripts/NIAF/calibrate_csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_sbatch.sh"
CPU_SCRIPT="$PROJECT_DIR/scripts/NIAF/test_csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_sbatch.sh"
...
MEMORY_CFG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_protocol_v4_run_r5.yaml"
OFF_CFG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_protocol_v4_run_r5.yaml"
RECOVERY_POLICY="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5_decision_policy_v1.json"
...
SMOKE_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v4_run_r5_smoke"
PILOT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v4_run_r5_pilot"
...
"$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-protocol-v4-runtime --policy_path "$RECOVERY_POLICY" --recovery_manifest "$RECOVERY_MANIFEST" --source_root "$PROJECT_DIR" --evidence_root "$RECOVERY_EVIDENCE_ROOT" >/dev/null
...
if [[ ! -e "$CALIBRATION_COMPLETION" \
  ...
  echo "ERROR: protocol-v4/run-r5 partial calibration is terminal; no retry submitted" >&2
  exit 1
fi

# A run-r5 control directory without a publication is never a new-attempt
...
for mode_spec in \
  ...
  if [[ -e "$control" && ! -e "$marker" ]]; then
    ...
    [[ "${#publication_recovery_completions[@]}" == "1" ]] || {
      echo "ERROR: protocol-v4/run-r5 $mode is terminal without one complete publication-recovery execution" >&2
      exit 1
    }
    [[ "$HAVE_CALIBRATION" == "1" ]] || {
...
if [[ -e "$PREREQUISITE_ROOT/pilot" && ! -e "$PILOT_READY" \
  ...
  [[ "$HAVE_SMOKE" == "1" \
    ...
    echo "ERROR: protocol-v4/run-r5 pilot is terminal without one complete publication-recovery execution" >&2
    exit 1
  }
  RECOVER_PILOT=1
fi
