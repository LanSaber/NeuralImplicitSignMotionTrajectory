#!/bin/bash
#SBATCH --job-name=csl_stage_c_protocol_v3_run_r4_cpu
...
GATE_DIR="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_stage_c_generator_adaptation_protocol_v3_run_r4_prerequisites/source_${SOURCE_GIT_HEAD,,}/cpu_gate"
...
for required in "$RECOVERY_POLICY" "$RECOVERY_MANIFEST" "$INCIDENT_ARCHIVE"; do
  [[ -f "$required" && ! -L "$required" ]] || {
    echo "ERROR: missing Stage-C protocol-v3/run-r4 recovery evidence: $required" >&2
    exit 1
  }
done
...
unset WANDB_API_KEY
CONFIG_AUDIT="$("$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-protocol-v3-configs --policy_path "$RECOVERY_POLICY" --recovery_manifest "$RECOVERY_MANIFEST" --source_root "$PROJECT_DIR" --evidence_root "$RECOVERY_EVIDENCE_ROOT" --memory_config "$MEMORY_CFG" --matched_off_config "$OFF_CFG")"
