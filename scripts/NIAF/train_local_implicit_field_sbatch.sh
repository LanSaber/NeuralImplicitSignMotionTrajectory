#!/bin/bash
#SBATCH --job-name=niaf_local_field
...
set -eo pipefail
trap 'echo "ERROR: train_local_implicit_field_sbatch.sh failed at line $LINENO with exit code $?" >&2' ERR
...
RUN_TAG="${RUN_TAG:-phoenix_local_amortized_adapter_text_pilot500_rawgt}"
DEVICE="${DEVICE:-auto}"
TEXT_DEVICE="${TEXT_DEVICE:-cpu}"
WANDB="${WANDB:-1}"
WANDB_MODE="${WANDB_MODE:-online}"
WANDB_PROJECT="${WANDB_PROJECT:-soke-niaf-local-implicit}"
...
TRAIN_CMD=(
  srun "$PYTHON_BIN" -m NIAF.continuous_sign_field.scripts.train_local_implicit_field
  --config "$CFG"
  --device "$DEVICE"
...
echo "Launching local implicit field: config=$CFG device=$DEVICE text_device=$TEXT_DEVICE W&B=$WANDB/$WANDB_MODE"
"${TRAIN_CMD[@]}"
