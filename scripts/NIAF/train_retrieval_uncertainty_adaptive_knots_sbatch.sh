#!/bin/bash
#SBATCH --job-name=niaf_uncert_knots
...
set -eo pipefail
trap 'echo "ERROR: train_retrieval_uncertainty_adaptive_knots_sbatch.sh failed at line $LINENO with exit code $?" >&2' ERR
...
CFG="${CFG:-NIAF/retrieval_confidence_field/configs/phoenix_retrieval_uncertainty_adaptive_knots_trainbalanced.yaml}"
RUN_TAG="${RUN_TAG:-phoenix_retrieval_uncertainty_adaptive_knots_trainbalanced}"
DEVICE="${DEVICE:-auto}"
...
WANDB_PROJECT="${WANDB_PROJECT:-soke-niaf-retrieval-uncertainty}"
...
TRAIN_CMD=(
  ...
  "$PYTHON_BIN" -m NIAF.retrieval_confidence_field.scripts.train_retrieval_adaptive_field
  --config "$CFG"
  --device "$DEVICE"
...
echo "Config: $CFG"
echo "Batch per rank: $BATCH_SIZE"
