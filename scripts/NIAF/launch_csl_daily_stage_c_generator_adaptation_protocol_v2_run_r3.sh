...
BATCH_SCRIPT="$PROJECT_DIR/scripts/NIAF/train_csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3_sbatch.sh"
SMOKE_SCRIPT="$PROJECT_DIR/scripts/NIAF/smoke_csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3_sbatch.sh"
CALIBRATION_SCRIPT="$PROJECT_DIR/scripts/NIAF/calibrate_csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3_sbatch.sh"
CPU_SCRIPT="$PROJECT_DIR/scripts/NIAF/test_csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3_sbatch.sh"
...
SMOKE_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v2_run_r3_smoke"
PILOT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v2_run_r3_pilot"
...
if [[ -e "$CPU_READY" ]]; then
  "$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-cpu \
    --cpu_gate "$CPU_READY" --source_git_head "$SOURCE_GIT_HEAD" \
...
if [[ ! -e "$CALIBRATION_COMPLETION" \
  ...
  echo "ERROR: protocol-v2/run-r3 partial calibration is terminal; no retry submitted" >&2
  exit 1
fi

# A run-r3 control directory without a publication is never a new-attempt
...
for mode_spec in \
  ...
  if [[ -e "$control" && ! -e "$marker" ]]; then
    ...
    [[ "${#publication_recovery_completions[@]}" == "1" ]] || {
      echo "ERROR: protocol-v2/run-r3 $mode is terminal without one complete publication-recovery execution" >&2
      exit 1
    }
    [[ "$HAVE_CALIBRATION" == "1" ]] || {
...
if [[ -e "$PREREQUISITE_ROOT/pilot" && ! -e "$PILOT_READY" \
  ...
  [[ "$HAVE_SMOKE" == "1" \
    ...
    echo "ERROR: protocol-v2/run-r3 pilot is terminal without one complete publication-recovery execution" >&2
    exit 1
  }
  RECOVER_PILOT=1
fi
