#!/bin/bash
#SBATCH --job-name=csl_stage_c_protocol_v4_run_r5_pilot
...
[[ "$SOURCE_GIT_HEAD" =~ ^[0-9a-fA-F]{40}$ ]] || { echo "ERROR: SOURCE_GIT_HEAD must be a full Git SHA1" >&2; exit 1; }
export SOURCE_GIT_HEAD="${SOURCE_GIT_HEAD,,}"
export STAGE_C_EXECUTION_MODE=pilot
bash "$PROJECT_DIR/scripts/NIAF/run_csl_daily_stage_c_generator_adaptation_protocol_v4_run_r5.sh"
