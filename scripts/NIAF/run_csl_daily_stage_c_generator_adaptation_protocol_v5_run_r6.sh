...
MEMORY_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_protocol_v5_run_r6"
OFF_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_protocol_v5_run_r6"
...
DECISION_POLICY="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_stage_c_generator_adaptation_protocol_v5_run_r6_decision_policy_v1.json"
...
CALIBRATION_LAUNCHER="$PROJECT_DIR/scripts/NIAF/calibrate_csl_daily_stage_c_generator_adaptation_protocol_v5_run_r6_sbatch.sh"
...
SMOKE_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v5_run_r6_smoke"
PILOT_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v5_run_r6_pilot"
...
for cfg, arm, mode, probability in (
    ...
    assert stage == {
        ...
        "active_stage_c": {
            "calibration_artifact_dir": "experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v5_run_r6",
...
assert normalized_generation(memory) == normalized_generation(r2_memory), (
    "protocol-v5 memory science differs from immutable protocol-v2/run-r3"
)
assert normalized_generation(off) == normalized_generation(r2_off), (
    "protocol-v5 matched-off science differs from immutable protocol-v2/run-r3"
)
...
# Protocol v5/run-r6 permits no new logical retry. Inspect the global mode
...
if [[ -e "$ATTEMPTS_ROOT" || -L "$ATTEMPTS_ROOT" ]]; then
  [[ -d "$ATTEMPTS_ROOT" && ! -L "$ATTEMPTS_ROOT" \
    ...
    echo "ERROR: protocol-v5/run-r6 attempt tree contains a nonregular entry" >&2
    exit 1
  }
fi
[[ "${#PREEXISTING_COMPLETIONS[@]}" -le 1 ]] || {
  echo "ERROR: protocol-v5/run-r6 has multiple completed executions" >&2
  exit 1
}
if [[ "${#PREEXISTING_COMPLETIONS[@]}" == "0" \
      && "${#PREEXISTING_ATTEMPT_ENTRIES[@]}" != "0" ]]; then
  echo "ERROR: protocol-v5/run-r6 forbids retry/resume after partial execution" >&2
  exit 1
fi
...
PY
  ...
  if [[ "$ACTIVE_OWNER_JOB" != "$SLURM_JOB_ID" \
    ...
    echo "ERROR: protocol-v5/run-r6 forbids replacing another logical job" >&2
    exit 1
  fi
  if [[ -e "$LEASE/TERMINAL.json" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
    echo "ERROR: protocol-v5/run-r6 terminal lease cannot create a new attempt" >&2
    exit 1
  fi
elif [[ -e "$LEASE" || -L "$LEASE" ]]; then
  echo "ERROR: protocol-v5/run-r6 active lease is malformed" >&2
  exit 1
elif [[ -e "$MODE_CONTROL_DIR" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v5/run-r6 prior control state forbids a new attempt" >&2
  exit 1
fi
if [[ -d "${LEASE}.history" \
      && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v5/run-r6 terminal lease history forbids retry" >&2
  exit 1
fi
...
# restart generation. The run-r6 precheck above forbids another job from
...
SOURCE_BINDING_FILES=(
  ...
  "$PROJECT_DIR/scripts/NIAF/run_csl_daily_stage_c_generator_adaptation_protocol_v5_run_r6.sh"
)
...
revalidate_source_binding() {
  ...
  "$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-protocol-v5-runtime --policy_path "$DECISION_POLICY" --recovery_manifest "$RECOVERY_MANIFEST" --source_root "$PROJECT_DIR" --evidence_root "$RECOVERY_EVIDENCE_ROOT" >/dev/null
