...
EXPECTED_SOURCE_REMOTE_BRANCH="codex/csl-daily-centered-generator-stage-c-v3-run-r4"
...
MEMORY_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_protocol_v3_run_r4"
OFF_EXPERIMENT="csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_protocol_v3_run_r4"
...
R3_MEMORY_CFG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_memory_protocol_v2_run_r3.yaml"
R3_OFF_CFG="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_matched_off_protocol_v2_run_r3.yaml"
...
DECISION_POLICY="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_stage_c_generator_adaptation_protocol_v3_run_r4_decision_policy_v1.json"
RECOVERY_MANIFEST="$PROJECT_DIR/NIAF/continuous_trajectory_field/configs/csl_daily_stage_c_generator_adaptation_protocol_v3_run_r4_recovery_evidence_v1.json"
RECOVERY_EVIDENCE_ROOT="/media/cvpr/haomian/NeuralImplicitSignMotionTrajectory"
INCIDENT_ARCHIVE="$RECOVERY_EVIDENCE_ROOT/experiments/NIAF/continuous_trajectory_field/csl_daily_stage_c_generator_adaptation_protocol_v2_run_r3_cpu.invalid_attempts/source_90cad3d3a7a09279b4a882e8c28a13b2a320ec1a_cpu143574_dependents143575_143577/ARCHIVE.json"
STAGER="$PROJECT_DIR/scripts/NIAF/stage_sentence_memory_train_val_only_node.sh"
CALIBRATION_LAUNCHER="$PROJECT_DIR/scripts/NIAF/calibrate_csl_daily_stage_c_generator_adaptation_protocol_v3_run_r4_sbatch.sh"
...
CALIBRATION_DIR="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v3_run_r4"
...
PREREQUISITE_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_stage_c_generator_adaptation_protocol_v3_run_r4_prerequisites/source_${SOURCE_GIT_HEAD,,}"
...
SMOKE_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v3_run_r4_smoke"
PILOT_RESULT_ROOT="$PROJECT_DIR/experiments/NIAF/continuous_trajectory_field/csl_daily_signtrajfield_v3_sentence_memory_stage_c_generator_adaptation_protocol_v3_run_r4_pilot"
...
EXPECTED_R3_MEMORY_CONFIG_SHA256="77e61c12a68f5f5bd60cc23355ca935fa5bf0a1ed777a9da440203417e3750cc"
EXPECTED_R3_OFF_CONFIG_SHA256="c36f5617d20d3ddc7386f8ec8d4811f58417fe716d86a5dc249c38a71258f880"
...
for required in "$PYTHON_BIN" "$MEMORY_CFG" "$OFF_CFG" \
  "$R3_MEMORY_CFG" "$R3_OFF_CFG" "$STAGE_B_CFG" \
  ...
  "$DECISION_HELPER" "$DECISION_POLICY" "$RECOVERY_MANIFEST" "$INCIDENT_ARCHIVE" \
  "$CALIBRATION_LAUNCHER"; do
  [[ -e "$required" ]] || { echo "ERROR: missing Stage-C prerequisite: $required" >&2; exit 1; }
done
[[ ! -e "$GLOBAL_HOLDOUT_SPEND" && ! -L "$GLOBAL_HOLDOUT_SPEND" ]] || {
...
require_sha256 "$R3_MEMORY_CFG" "$EXPECTED_R3_MEMORY_CONFIG_SHA256" "protocol-v2/run-r3 memory config"
require_sha256 "$R3_OFF_CFG" "$EXPECTED_R3_OFF_CONFIG_SHA256" "protocol-v2/run-r3 matched-off config"
...
require_holdout_unspent() {
  "$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-unspent \
    --spend_marker "$GLOBAL_HOLDOUT_SPEND" >/dev/null
}
require_holdout_unspent

"$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-protocol-v3-cpu --cpu_gate "$CPU_GATE_READY" --source_git_head "$SOURCE_GIT_HEAD" --source_remote_ref "origin/$SOURCE_REMOTE_BRANCH" --source_remote_head "$REMOTE_HEAD" --policy_path "$DECISION_POLICY" --recovery_manifest "$RECOVERY_MANIFEST" --source_root "$PROJECT_DIR" --evidence_root "$RECOVERY_EVIDENCE_ROOT" --memory_config "$MEMORY_CFG" --matched_off_config "$OFF_CFG"
...
"$PYTHON_BIN" - "$MEMORY_CFG" "$OFF_CFG" "$PROJECT_DIR" \
  ...
  "$EXPECTED_STAGE_B_ARCHITECTURE_IDENTITY" "$EXPECTED_V2_CHECKPOINT_SHA256" \
  "$R3_MEMORY_CFG" "$R3_OFF_CFG" <<'PY'
import copy
import hashlib
import json
import sys
...
r2_memory = load_config(Path(sys.argv[10]))
r2_off = load_config(Path(sys.argv[11]))
for cfg, arm, mode, probability in (
    ...
    assert stage == {
        ...
        "active_stage_c": {
            "calibration_artifact_dir": "experiments/NIAF/continuous_trajectory_field/csl_daily_sentence_memory_relevance_calibration_stage_c_generator_adaptation_protocol_v3_run_r4",
...
assert normalized_generation(memory) == normalized_generation(r2_memory), (
    "protocol-v3 memory science differs from immutable protocol-v2/run-r3"
)
assert normalized_generation(off) == normalized_generation(r2_off), (
    "protocol-v3 matched-off science differs from immutable protocol-v2/run-r3"
)
...
if [[ "$EXECUTION_MODE" == "smoke" ]]; then
  PILOT_ROOT="$SMOKE_RESULT_ROOT"
else
  PILOT_ROOT="$PILOT_RESULT_ROOT"
fi
...
# Protocol v3/run-r4 permits no new logical retry. Inspect the global mode
# namespace before lease acquisition, because the generic execution-control
# helper intentionally supports older protocols that may replace a terminal
# zero-science owner.  Here a different/stale owner, terminal lease history,
# or any partial attempt is terminal.  The only allowed continuations are a
# scheduler-proven requeue of the same active job before an attempt exists, or
# publication-only recovery of one already complete execution.
mapfile -t PREEXISTING_COMPLETIONS < <(
  if [[ -d "$ATTEMPTS_ROOT" && ! -L "$ATTEMPTS_ROOT" ]]; then
    find "$ATTEMPTS_ROOT" -mindepth 4 -maxdepth 4 \
      -type f -name COMPLETE.json -print | sort
  fi
)
mapfile -t PREEXISTING_ATTEMPT_ENTRIES < <(
  if [[ -d "$ATTEMPTS_ROOT" && ! -L "$ATTEMPTS_ROOT" ]]; then
    find "$ATTEMPTS_ROOT" -mindepth 1 -print | sort
  fi
)
if [[ -e "$ATTEMPTS_ROOT" || -L "$ATTEMPTS_ROOT" ]]; then
  [[ -d "$ATTEMPTS_ROOT" && ! -L "$ATTEMPTS_ROOT" \
     && -z "$(find "$ATTEMPTS_ROOT" -mindepth 1 \
       \( -type l -o \( ! -type f ! -type d \) \) -print -quit)" ]] || {
    echo "ERROR: protocol-v3/run-r4 attempt tree contains a nonregular entry" >&2
    exit 1
  }
fi
[[ "${#PREEXISTING_COMPLETIONS[@]}" -le 1 ]] || {
  echo "ERROR: protocol-v3/run-r4 has multiple completed executions" >&2
  exit 1
}
if [[ "${#PREEXISTING_COMPLETIONS[@]}" == "0" \
      && "${#PREEXISTING_ATTEMPT_ENTRIES[@]}" != "0" ]]; then
  echo "ERROR: protocol-v3/run-r4 forbids retry/resume after partial execution" >&2
  exit 1
fi
...
print(owner.get("slurm_job_id", ""))
PY
)"
  if [[ "$ACTIVE_OWNER_JOB" != "$SLURM_JOB_ID" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
    echo "ERROR: protocol-v3/run-r4 forbids replacing another logical job" >&2
    exit 1
  fi
  if [[ -e "$LEASE/TERMINAL.json" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
    echo "ERROR: protocol-v3/run-r4 terminal lease cannot create a new attempt" >&2
    exit 1
  fi
elif [[ -e "$LEASE" || -L "$LEASE" ]]; then
  echo "ERROR: protocol-v3/run-r4 active lease is malformed" >&2
  exit 1
elif [[ -e "$MODE_CONTROL_DIR" \
        && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v3/run-r4 prior control state forbids a new attempt" >&2
  exit 1
fi
if [[ -d "${LEASE}.history" \
      && "${#PREEXISTING_COMPLETIONS[@]}" == "0" ]]; then
  echo "ERROR: protocol-v3/run-r4 terminal lease history forbids retry" >&2
  exit 1
fi
...
# restart generation. The run-r4 precheck above forbids another job from
# replacing it unless one complete execution needs publication-only recovery.
...
SOURCE_BINDING_FILES=(
  ...
  "$DECISION_POLICY" "$RECOVERY_MANIFEST" "$INCIDENT_ARCHIVE" "$CALIBRATION_LAUNCHER"
  "$R3_MEMORY_CFG" "$R3_OFF_CFG"
  ...
  "$PROJECT_DIR/scripts/NIAF/run_csl_daily_stage_c_generator_adaptation_protocol_v3_run_r4.sh"
)
EXPECTED_SOURCE_BINDING_FILE_COUNT=30
if [[ "$EXECUTION_MODE" == "pilot" ]]; then
  SOURCE_BINDING_FILES+=("$SMOKE_READY")
  EXPECTED_SOURCE_BINDING_FILE_COUNT=31
fi
...
revalidate_source_binding() {
  [[ "$(git rev-parse HEAD)" == "${SOURCE_GIT_HEAD,,}" \
     ...
     && ! -L "$GLOBAL_HOLDOUT_SPEND" ]] || {
    echo "ERROR: Stage-C source/scope changed during paired execution" >&2
    return 1
  }
  local current_remote
  current_remote="$(timeout 120s git ls-remote --heads origin "refs/heads/$SOURCE_REMOTE_BRANCH" | awk 'NR == 1 {print $1}')"
  [[ "$current_remote" == "${REMOTE_HEAD,,}" ]] || {
    echo "ERROR: Stage-C remote source head changed during paired execution" >&2
    return 1
  }
  "$PYTHON_BIN" -m "$PREREQUISITE_MODULE" validate-protocol-v3-runtime --policy_path "$DECISION_POLICY" --recovery_manifest "$RECOVERY_MANIFEST" --source_root "$PROJECT_DIR" --evidence_root "$RECOVERY_EVIDENCE_ROOT" >/dev/null
  "$PYTHON_BIN" - "$SOURCE_BINDING_MANIFEST" "$EXECUTION_MODE" \
    "$SMOKE_READY" "$SMOKE_RESULT_ROOT" "$SOURCE_GIT_HEAD" \
    "$PAIR_CONSTRAINT" "$MEMORY_CFG" "$OFF_CFG" "$CPU_GATE_READY" \
    "$CALIBRATION_COMPLETION" "$DECISION_POLICY" \
    "origin/$SOURCE_REMOTE_BRANCH" "$REMOTE_HEAD" \
    "$EXPECTED_SOURCE_BINDING_FILE_COUNT" <<'PY'
...
expected_fields = {
    ...
    "source_remote_ref", "source_remote_head", "files", "development_only",
    "non_authorizing",
}
if mode == "pilot":
    expected_fields.add("prior_one_update_smoke")
if (
    set(value) != expected_fields
    or value.get("schema_name") != "signtrajfield_stage_c_source_binding"
    or value.get("schema_version") != 1
    or value.get("execution_mode") != mode
    or value.get("source_git_head") != sys.argv[5]
    or value.get("source_remote_ref") != sys.argv[12]
    or value.get("source_remote_head") != sys.argv[13]
...
if len(value.get("files", {})) != int(sys.argv[14]):
...
if mode == "pilot":
    current = validate_smoke(
        smoke_ready=Path(sys.argv[3]),
        smoke_root=Path(sys.argv[4]),
        source_git_head=sys.argv[5],
        pair_constraint=sys.argv[6],
    )
    expected_current = {
        "memory_config_sha256": sha256_file(Path(sys.argv[7])),
        "matched_off_config_sha256": sha256_file(Path(sys.argv[8])),
        "cpu_gate_sha256": sha256_file(Path(sys.argv[9])),
        "calibration_completion_sha256": sha256_file(Path(sys.argv[10])),
        "decision_policy_sha256": sha256_file(Path(sys.argv[11])),
        "source_remote_ref": sys.argv[12],
        "source_remote_head": sys.argv[13],
    }
    if value.get("prior_one_update_smoke") != current or any(
        current.get(name) != expected for name, expected in expected_current.items()
    ):
        raise SystemExit(
            "ERROR: prior one-update smoke differs from the active Stage-C foundation"
        )
else:
    if "prior_one_update_smoke" in value:
        raise SystemExit("ERROR: smoke source binding contains a self-prerequisite")
PY
}
...
"$PYTHON_BIN" - "$SOURCE_BINDING_MANIFEST" "$SOURCE_GIT_HEAD" \
  "origin/$SOURCE_REMOTE_BRANCH" "$REMOTE_HEAD" "$EXECUTION_MODE" \
  "$SMOKE_READY" "$SMOKE_RESULT_ROOT" "$PAIR_CONSTRAINT" \
  "$EXPECTED_SOURCE_BINDING_FILE_COUNT" "${SOURCE_BINDING_FILES[@]}" <<'PY'
import hashlib, json, sys
from pathlib import Path
...
files = [Path(value).resolve() for value in sys.argv[10:]]
if len(files) != expected_count or len(files) != len(set(files)) or any(not value.is_file() or value.is_symlink() for value in files):
...
if mode == "pilot":
    prior_smoke = validate_smoke(
        smoke_ready=Path(sys.argv[6]),
        smoke_root=Path(sys.argv[7]),
        source_git_head=sys.argv[2],
        pair_constraint=sys.argv[8],
    )
    ready_path = str(Path(sys.argv[6]).resolve())
    if payload["files"].get(ready_path) != prior_smoke["ready_sha256"]:
        raise SystemExit("ERROR: source binding does not hash-bind smoke READY")
    payload["prior_one_update_smoke"] = prior_smoke
elif mode != "smoke":
    raise SystemExit("ERROR: invalid Stage-C source-binding execution mode")
...
source_binding = json.loads(Path(sys.argv[25]).read_text(encoding="utf-8"))
payload = {
...
if mode == "pilot":
    prior_smoke = source_binding.get("prior_one_update_smoke")
    if not isinstance(prior_smoke, dict):
        raise SystemExit("ERROR: pilot LAUNCH lacks its smoke prerequisite")
    payload["prior_one_update_smoke"] = prior_smoke
elif "prior_one_update_smoke" in source_binding:
    raise SystemExit("ERROR: smoke LAUNCH cannot bind a self-prerequisite")
...
source_binding = json.loads(
    (network / "SOURCE_BINDING.json").read_text(encoding="utf-8")
)
execution_artifacts = {
...
if mode == "pilot":
    prior_smoke = source_binding.get("prior_one_update_smoke")
    if not isinstance(prior_smoke, dict):
        raise SystemExit("ERROR: pilot COMPLETE lacks its smoke prerequisite")
    payload["prior_one_update_smoke"] = prior_smoke
elif "prior_one_update_smoke" in source_binding:
    raise SystemExit("ERROR: smoke COMPLETE cannot bind a self-prerequisite")
...
prior_smoke = execution_complete.get("prior_one_update_smoke")
if (
...
if mode == "pilot":
    if (
        not isinstance(prior_smoke, dict)
        or launch.get("prior_one_update_smoke") != prior_smoke
        or decision.get("prior_one_update_smoke") != prior_smoke
    ):
        raise SystemExit(
            "ERROR: pilot publication cannot bind its one-update smoke prerequisite"
        )
elif any(
    "prior_one_update_smoke" in value
    for value in (execution_complete, launch, decision)
):
    raise SystemExit("ERROR: smoke publication contains a self-prerequisite")
complete = {
...
if mode == "pilot":
    complete["prior_one_update_smoke"] = prior_smoke
...
if mode == "pilot":
    ready["prior_one_update_smoke"] = prior_smoke
