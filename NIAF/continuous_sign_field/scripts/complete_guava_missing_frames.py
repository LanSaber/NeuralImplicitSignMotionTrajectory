...
fits the rotation-aware Slerp scaffold used by the current NIAF field pipeline
...
from flow.dataset.prepare_tracked_guard_dataset import frame_compact
...
from NIAF.oracle_smplx_field.models.baselines import SlerpBaseline
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Complete GUAVA-discarded frames with the NIAF rotation-aware continuous scaffold."
    )
...
def validate_frame_trace(trace):
    ...
    if set(kept_indices) & set(discarded_indices):
        raise ValueError("Trace marks at least one original frame as both kept and discarded")
...
def complete_motion_slerp(observed_motion, observed_indices, total_frames):
    ...
    if observed_motion.ndim != 2 or observed_motion.shape[1] != COMPACT_DIM:
        raise ValueError(f"Expected observed motion [K,{COMPACT_DIM}], got {observed_motion.shape}")
    ...
    observed_rot6d = compact_axis_angle_to_rot6d_torch(torch.from_numpy(observed_motion)).float()
    ...
    if len(observed_motion) == 1:
        ...
        completed_rot6d = baseline.predict(torch.from_numpy(tau_all)).cpu().numpy().astype(np.float32)
...
def finalize_stats(stats):
    if stats["count"] <= 0:
        raise RuntimeError("No training frames were available for normalization statistics")
...
def process_sequence(sent_dir, split, args, source_row):
    ...
    if len(kept) < int(args.min_observed_frames):
        raise ValueError(f"Only {len(kept)} retained frames; need {args.min_observed_frames}")
    ...
    if out_path.is_file() and not args.overwrite:
        ...
        left_valid, right_valid, left_score, right_score, hand_distance = hand_validity_from_trace(
            total, discarded
        )
    ...
    for split in splits:
        ...
        for sent_dir in tqdm(candidates, desc=f"complete GUAVA {split}"):
            try:
                ...
                warnings.warn(f"Skipping {split}/{sent_dir.name}: {exc}", RuntimeWarning)
                skipped.append(
                    {
                        "split": split,
                        "name": sent_dir.name,
