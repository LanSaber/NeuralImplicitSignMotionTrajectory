...
from NIAF.continuous_sign_field.guava_gap import (
    CONDITION_DIM,
    apply_bounded_correction,
    eligible_mask_from_condition,
    gap_condition_features,
)
from NIAF.continuous_sign_field.guava_self_supervision import build_masked_guava_view
from NIAF.continuous_sign_field.guava_temporal import (
    frames_from_window_mask,
    gap_local_fk_temporal_losses,
    masked_joint_magnitude,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Mask-aware GUAVA meta-implicit fine-tuning.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(
            "NIAF/continuous_sign_field/configs/guava_mask_aware_meta_finetune.yaml"
        ),
    )
...
def pilot_groups(path):
    ...
    if not groups:
        raise FileNotFoundError(f"No pilot fits found in {path}; cannot form leakage exclusion")
    return groups


def validation_groups(groups, fraction, seed):
    groups = sorted(set(groups))
    count = min(max(int(round(len(groups) * float(fraction))), 1), max(len(groups) - 1, 1))
    ranked = sorted(
        groups,
...
def cache_item(
    ...
    reference_axis = load_compact_sequence(frame_files_for(reference_path)).astype(np.float32)
    target = compact_axis_angle_to_rot6d_torch(torch.from_numpy(reference_axis)).numpy().astype(np.float32)
    if scaffold.shape != target.shape or scaffold.shape != (len(observed), COMPACT6D_DIM):
        raise ValueError(
...
class GuavaMaskDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.samples[index] if self.samples is not None else self._load(self.rows[index])
...
def calibrate_bounds(rows, cfg):
    ...
    for row in tqdm(rows, desc="calibrate training-only bounds", leave=False):
        ...
        angles = torch.linalg.norm(
            matrix_to_axis_angle(scaffold_matrix.transpose(-1, -2) @ target_matrix), dim=-1
        ).numpy()
...
def transplant_parent(model, parent_state):
    ...
    for key, value in parent_state.items():
        ...
        if state[key].shape == value.shape:
            ...
            continue
        if key == "net.0.linear.weight" and state[key].shape[0] == value.shape[0] and state[key].shape[1] > value.shape[1]:
            widened = torch.zeros_like(state[key])
            widened[:, : value.shape[1]] = value
            state[key] = widened
            expanded.append(
                {"key": key, "old_shape": list(value.shape), "new_shape": list(widened.shape)}
            )
            continue
        raise ValueError(f"Cannot transplant {key}: parent {value.shape}, new {state[key].shape}")
...
def fk_parts_fixed(fk, compact6d, chunk_size):
    ...
    for start in range(0, len(compact6d), int(chunk_size)):
        ...
        if actual < int(chunk_size):
            value = torch.cat([value, value[-1:].expand(int(chunk_size) - actual, -1)], dim=0)
...
def masked_guava_self_supervision_losses(
    ...
    metrics = {
        ...
        + float(stats["masked_frames"]) / max(float(stats["available_observed_frames"]), 1.0),
    }
    if view is None:
        return zero, metrics
...
def training_losses(
    ...
    _raw, pred = forward_prediction(model, batch, text_tokens, text_mask, bounds, strength=1.0)
    target = batch["target"]
    mask = batch["eligible"]
    loss_cfg = cfg["loss"]
...
def select_validation_alpha(metrics, alphas, cfg):
    ...
    if passed:
        ...
        checkpoint_score = float(
            metrics[f"alpha_{selected:.2f}"]["fk_jerk_p95_mps3"]
        )
    else:
...
def evaluate(model, fk, loader, text_tokens, text_mask, bounds, cfg, device):
    ...
    sums = {alpha: {key: 0.0 for key in ("body", "lhand", "rhand", "wholebody")} for alpha in alphas}
    ...
    for raw_batch in tqdm(loader, desc="validation", leave=False):
        ...
        raw, _unit = forward_prediction(model, batch, text_tokens, text_mask, bounds, strength=1.0)
        mask = batch["eligible"]
...
def save_checkpoint(path, model, optimizer, scheduler, epoch, global_step, cfg, bounds, validation, manifest, parent_info):
    path = Path(path)
...
def main():
        ...
        json.dumps(
            {
                ...
                "train_eligible_frames": sum(row["eligible_frames"] for row in train_rows),
    ...
    fk = DifferentiableSMPLXForward(
        model_dir=cfg["metrics"].get("model_dir", "deps/smpl_models"),
    ...
    append_jsonl(
        ...
        {"epoch": best_epoch, "global_step": global_step, "validation": initial_validation},
    )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if patience > 0 and stale_epochs >= patience:
            ...
            break
    print(json.dumps({"best_epoch": best_epoch, "best_checkpoint_score": best_score}, indent=2))
