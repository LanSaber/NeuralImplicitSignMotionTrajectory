...
from NIAF.continuous_sign_field.guava_self_supervision import (
...
from NIAF.continuous_sign_field.scripts.train_guava_mask_aware_meta import (
    GuavaMaskDataset,
    MeanTracker,
    append_jsonl,
...
def parse_args():
    parser = argparse.ArgumentParser(description="SOKE-free masked-GUAVA fine-tuning.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(
            "NIAF/continuous_sign_field/configs/guava_self_only_meta_c2_fk_jerk.yaml"
        ),
    )
...
def prepare_manifest_guava_only(cfg, out_dir):
    ...
    rows = []
    for path in tqdm(candidates, desc="prepare SOKE-free GUAVA cache"):
...
def calibrate_guava_only_bounds(rows, cfg):
    ...
    for row in tqdm(rows, desc="calibrate masked-GUAVA bounds", leave=False):
        with np.load(row["cache_path"], allow_pickle=False) as data:
            scaffold = torch.from_numpy(data["scaffold"].astype(np.float32)).unsqueeze(0)
            observed = torch.from_numpy(data["observed_mask"].astype(np.bool_)).unsqueeze(0)
            eligible = torch.from_numpy(data["eligible_mask"].astype(np.bool_)).unsqueeze(0)
            condition = torch.from_numpy(data["condition"].astype(np.float32)).unsqueeze(0)
        ...
        for view_index in range(views):
            ...
            expression = torch.abs(target[..., 246:] - synthetic[..., 246:])[mask].numpy()
    ...
    if sampled_frames == 0 or any(not chunks for chunks in values.values()):
        raise RuntimeError("Could not sample retained GUAVA frames for bound calibration")
...
def real_gap_safety_losses(
    ...
    if not bool(safety_cfg.get("enabled", True)) or float(safety_cfg.get("weight", 0.0)) <= 0:
        return zero, metrics
    ...
    metrics.update({f"real_safety_{key}": value for key, value in temporal_metrics.items()})
...
def select_guava_only_alpha(metrics, alphas, cfg):
    ...
    minimum_improvement = float(
        eval_cfg.get("min_masked_guava_mpjpe_improvement_percent", 1.0)
    ) / 100.0
    ...
    masked_jerk_limit = baseline["masked_guava_fk_jerk_p95_mps3"] * (
        1.0 + float(eval_cfg.get("max_masked_guava_jerk_increase_percent", 25.0)) / 100.0
    )
    ...
    if accepted:
        ...
        tolerance = float(eval_cfg.get("mpjpe_tolerance_from_best_percent", 2.0)) / 100.0
        near_best = [
            alpha
            for alpha in accepted
    ...
    return selected, {
        ...
        * (
            diagnostic_score
            / baseline["masked_guava_mpjpe_wholebody_m"]
            - 1.0
        ),
...
def evaluate_guava_only(model, fk, loader, text_tokens, text_mask, bounds, cfg, device):
    ...
    if not any(math.isclose(alpha, 0.0, abs_tol=1.0e-12) for alpha in alphas):
        raise ValueError("GUAVA-only validation alpha_grid must include 0.0 safe fallback")
    ...
    for raw_batch in tqdm(loader, desc="SOKE-free validation", leave=False):
    ...
    for alpha in alphas:
        ...
        for bucket in buckets:
            ...
            metrics[key][f"masked_guava_gap_{name}_mpjpe_wholebody_m"] = (
                bucket_sums[alpha][name] / max(joint_count, 1)
            )
        self_values = (
...
def main():
    ...
    if args.resume:
        ...
        if checkpoint.get("uses_soke_training_targets", True):
            raise ValueError("Cannot resume GUAVA-only training from a SOKE-target checkpoint")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        candidate_score = float(
            validation["diagnostic_best_safe_mpjpe_wholebody_m"]
        )
        ...
        if patience > 0 and stale_epochs >= patience:
            print(f"Early stopping after {stale_epochs} epochs without safe improvement")
            break
    print(
        json.dumps(
            {
