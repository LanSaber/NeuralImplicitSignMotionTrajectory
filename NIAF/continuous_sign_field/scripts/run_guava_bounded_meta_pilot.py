...
from flow.dataset.prepare_how2sign_soke_dataset import frame_files_for, load_compact_sequence
...
from NIAF.continuous_sign_field.scaffold import build_sequence_scaffold, normalized_time_grid
from NIAF.continuous_sign_field.scripts.evaluate_guava_completion_jerk import (
    contiguous_true_runs,
    distribution_stats,
    joint_parts_chunked,
)
from NIAF.continuous_sign_field.scripts.train_meta_implicit_field import build_meta_model
from NIAF.continuous_sign_field.scripts.train_residual_flow import build_text_encoder
from NIAF.oracle_smplx_field.geometry.smplx_fk import DifferentiableSMPLXForward


DEFAULT_SOFT_CHECKPOINT = Path(
    "experiments/NIAF/continuous_sign_field/"
    "smpl_samples_meta_implicit_soft_recon_stride8_fk_smooth/checkpoints/best.pt"
)
DEFAULT_TEMPORAL_CHECKPOINT = Path(
    "experiments/NIAF/continuous_sign_field/"
    "smpl_samples_meta_implicit_gt_anchor_stride8_fk_temporal/checkpoints/best.pt"
)
DEFAULT_REFERENCE_ROOT = Path("/media/cvpr/haomian/data/SOKE/How2Sign")
DEFAULT_OUT_DIR = Path("visualize/NIAF/continuous_sign_field/guava_bounded_meta_pilot")
DEFAULT_FPS_SUMMARY = Path(
    "visualize/NIAF/continuous_sign_field/guava_linear_siren_jerk_compare/render_summary.json"
)
...
def parse_args():
    ...
    parser.add_argument("--temporal_checkpoint", type=Path, default=DEFAULT_TEMPORAL_CHECKPOINT)
...
def load_completion(path):
    with np.load(path, allow_pickle=False) as data:
        ...
        rot6d = (
            ...
            else compact_axis_angle_to_rot6d_torch(torch.from_numpy(motion)).numpy().astype(np.float32)
        )
        observed = data["observed_mask"].astype(np.bool_)
        method = scalar_string(data["completion_method"]) if "completion_method" in data.files else ""
...
def calibrate_bounds(cfg, percentile=95.0, stride=8):
    ...
    if not data_dir:
        raise ValueError("Checkpoint config has no data.data_dir for residual-bound calibration")
    for motion in calibration_sequences(data_dir):
        ...
        query = ~anchor_mask
        target_matrix = rotation_6d_to_matrix(target[:, :246].reshape(len(target), 41, 6))
        scaffold_matrix = rotation_6d_to_matrix(scaffold[:, :246].reshape(len(target), 41, 6))
        relative = scaffold_matrix.transpose(-1, -2) @ target_matrix
        angles = torch.linalg.norm(matrix_to_axis_angle(relative), dim=-1).detach().cpu().numpy()
        ...
        expression = torch.abs(target[:, 246:] - scaffold[:, 246:]).detach().cpu().numpy()
...
def bound_prediction(scaffold, raw_prediction, observed, bounds):
    ...
    if scaffold.shape != raw_prediction.shape or scaffold.ndim != 2:
        raise ValueError(f"Expected matching [T,256] tensors, got {scaffold.shape}, {raw_prediction.shape}")
    envelope_np = gap_envelope(observed)
    envelope = torch.from_numpy(envelope_np).to(device=scaffold.device, dtype=scaffold.dtype)
    scaffold_matrix = rotation_6d_to_matrix(scaffold[:, :246].reshape(len(scaffold), 41, 6))
    raw_matrix = rotation_6d_to_matrix(raw_prediction[:, :246].reshape(len(scaffold), 41, 6))
    ...
    observed_tensor = torch.from_numpy(np.asarray(observed, dtype=np.bool_)).to(scaffold.device)
    ...
    for key, index in ROTATION_GROUPS.items():
        ...
        cap_stats[key] = {
            ...
            "raw_angle_mean_deg": float(torch.rad2deg(selected).mean().cpu()) if selected.numel() else None,
            "raw_angle_p95_deg": float(torch.quantile(torch.rad2deg(selected), 0.95).cpu()) if selected.numel() else None,
            "fraction_clipped": float((selected > selected_caps).float().mean().cpu()) if selected.numel() else None,
        }
    cap_stats["bracketed_corrected_frames"] = int(eligible.sum().cpu())
    cap_stats["endpoint_hold_missing_frames"] = int(((~np.asarray(observed)) & (envelope_np == 0)).sum())
...
def predict_meta(model, scaffold_np, observed, bounds, text_tokens, text_mask, device):
    ...
    tau = normalized_time_grid(lengths, max_len=len(scaffold), device=device, dtype=scaffold.dtype)
...
def reference_motion(path, reference_root, expected_shape):
    ...
    if motion.shape != tuple(expected_shape):
        raise ValueError(f"Reference shape mismatch for {path.stem}: {motion.shape} vs {expected_shape}")
...
def difference(values, order, fps):
    return np.diff(np.asarray(values, dtype=np.float64), n=int(order), axis=0) * float(fps) ** int(order)
...
def evaluate_methods(
    ...
    combined = np.concatenate([reference_axis] + [methods_axis[key] for key in method_names], axis=0)
    ...
    for method in method_names:
        method_parts[method] = {key: value[offset : offset + length] for key, value in parts.items()}
        offset += length
    ...
    for method in method_names:
        ...
        expression_error = np.abs(methods_rot6d[method][:, 246:] - reference_rot6d[:, 246:])
        ...
        for group, frame_mask in groups.items():
            ...
            for rotation_group, index in ROTATION_GROUPS.items():
                ...
                group_metrics[f"geodesic_{rotation_group}_deg"] = distribution_stats(values)
                flat_values[method][group][f"geodesic_{rotation_group}_deg"] = values.reshape(-1)
            group_metrics["expression_mae"] = distribution_stats(expression_error[frame_mask])
            flat_values[method][group]["expression_mae"] = expression_error[frame_mask].reshape(-1)
            for part in ("body", "lhand", "rhand", "wholebody"):
                error = np.linalg.norm(method_parts[method][part] - reference_parts[part], axis=-1)
        ...
        for label, order in (("velocity", 1), ("acceleration", 2), ("jerk", 3)):
            ...
            flat_values[method]["temporal"][f"{label}_vector_error"] = vector_error.reshape(-1)
            flat_values[method]["temporal"][f"{label}_reference_magnitude"] = ref_magnitude.reshape(-1)
            flat_values[method]["temporal"][f"{label}_prediction_magnitude"] = pred_magnitude.reshape(-1)
    ...
    for row in rows:
        for method in method_names:
            ...
            for label in ("velocity", "acceleration", "jerk"):
                values[f"{label}_vector_error_mean"] = row["metrics"][method]["temporal"][label][
                    "vector_error"
                ]["mean"]
                values[f"{label}_magnitude_ratio"] = row["metrics"][method]["temporal"][label][
                    "prediction_to_reference_mean_ratio"
                ]
            flat.append(values)
    return flat
...
def main():
    args = parse_args()
    missing_paths = [str(path) for path in (args.input + [args.soft_checkpoint, args.temporal_checkpoint]) if not path.is_file()]
    if missing_paths:
    ...
    soft_checkpoint = torch.load(args.soft_checkpoint, map_location="cpu", weights_only=False)
    soft_cfg = soft_checkpoint.get("config") or {}
    text_encoder = build_text_encoder(soft_cfg, text_device)
    text_tokens, text_mask = text_encoder.encode_tokens([""])
    text_tokens = text_tokens.to(device)
    text_mask = text_mask.to(device)
    soft_model, soft_cfg, soft_checkpoint = load_frozen_model(
        args.soft_checkpoint, text_encoder.text_dim, device
    )
    temporal_model, temporal_cfg, temporal_checkpoint = load_frozen_model(
        args.temporal_checkpoint, text_encoder.text_dim, device
    )
    ...
    for path in tqdm(args.input, desc="bounded meta pilot"):
        ...
        reference_axis, reference_path = reference_motion(path, args.reference_root, slerp_axis.shape)
        reference_rot6d = compact_axis_angle_to_rot6d_torch(
            torch.from_numpy(reference_axis)
        ...
        raw_temporal, bounded_temporal, temporal_cap_stats, temporal_envelope = predict_meta(
            temporal_model, slerp_rot6d, observed, bounds, text_tokens, text_mask, device
        )
        ...
        row = {
            ...
            "observed_exact_max_abs": {
                "soft_rot6d": float(np.max(np.abs(bounded_soft[observed] - slerp_rot6d[observed]))),
                "temporal_rot6d": float(np.max(np.abs(bounded_temporal[observed] - slerp_rot6d[observed]))),
                "soft_axis": float(np.max(np.abs(soft_axis[observed] - slerp_axis[observed]))),
                "temporal_axis": float(np.max(np.abs(temporal_axis[observed] - slerp_axis[observed]))),
            },
            "metrics": metrics,
        }
        rows.append(row)
