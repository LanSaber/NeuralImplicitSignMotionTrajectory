...
from NIAF.continuous_sign_field.scripts.run_guava_bounded_meta_pilot import (
    evaluate_methods,
...
DEFAULT_CHECKPOINT = Path(
    "experiments/NIAF/continuous_sign_field/guava_mask_aware_meta_finetune/checkpoints/best.pt"
)
DEFAULT_FROZEN_FITS = Path(
...
DEFAULT_OUT_DIR = Path(
    "visualize/NIAF/continuous_sign_field/guava_mask_aware_meta_pilot"
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate mask-aware GUAVA meta fine-tuning.")
...
def predict(
    ...
    tau = normalized_time_grid(lengths, len(scaffold), device=device, dtype=scaffold.dtype)
...
def correction_stats(scaffold, prediction, condition, observed):
    from flow.smplx_features import matrix_to_axis_angle, rotation_6d_to_matrix
    ...
    values = torch.rad2deg(
        torch.linalg.norm(
            matrix_to_axis_angle(scaffold_matrix.transpose(-1, -2) @ pred_matrix), dim=-1
        )
    ).numpy()[eligible]
    return {
        ...
        "rotation_correction_p95_deg": float(np.percentile(values, 95.0)) if values.size else 0.0,
...
def main():
    args = parse_args()
    missing = [str(path) for path in args.input + [args.checkpoint] if not path.is_file()]
    if missing:
    ...
    alpha_source = "checkpoint_selected_alpha" if args.alpha is None else "command_line_override"
    ...
    text_tokens, text_mask = blank_text_embedding(
        ...
        precomputed_path=cfg.get("data", {}).get("blank_text_cache"),
    )
    ...
    for path in tqdm(args.input, desc="held-out mask-aware pilot"):
        ...
        exact = {
            ...
            "axis": float(np.max(np.abs(finetuned_axis[observed] - slerp_axis[observed]))),
        }
        rows.append(
            {
                "sequence": path.stem,
    ...
    with (args.out_dir / "per_sequence_metrics.jsonl").open("w", encoding="utf-8") as handle:
        for row in rows:
