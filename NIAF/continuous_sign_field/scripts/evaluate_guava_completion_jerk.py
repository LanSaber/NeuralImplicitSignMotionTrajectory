...
wrist-relative, matching the existing NIAF FK evaluation convention.
"""
...
DEFAULT_OUT_DIR = Path("visualize/NIAF/continuous_sign_field/guava_completion_jerk")
DEFAULT_CURVE_MANIFEST = Path(
    "visualize/NIAF/continuous_sign_field/guava_completion_compare/render_summary.json"
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare reference and GUAVA-completed FK jerk magnitude.")
...
def joint_parts_chunked(fk, motion, device, batch_size):
    ...
    for start in range(0, len(motion), batch_size):
        ...
        for part in PART_NAMES:
            chunks[part].append(parts[part][:valid].detach().cpu().numpy().astype(np.float32))
...
def plot_curve(path, name, fps, gt_frame_mag, fit_frame_mag, touches_filled):
    ...
    axis.plot(time, np.maximum(gt_frame_mag, 1e-6), color="#d69b42", linewidth=1.5, label="Dense SOKE reference")
    axis.plot(time, np.maximum(fit_frame_mag, 1e-6), color="#42bb78", linewidth=1.4, label="GUAVA completion")
...
def evaluate_sequence(row, split, args, fk, device, wanted_curves, collector):
    ...
    if not gt_pose_dir.is_dir():
        raise FileNotFoundError(f"Missing dense reference pose directory: {gt_pose_dir}")
    ...
    if observed_mask.shape != (len(fitted_motion),):
        raise ValueError(f"Invalid observed mask shape for {name}: {observed_mask.shape}")
    ...
    parts = joint_parts_chunked(fk, combined, device=device, batch_size=args.fk_batch_size)
    ...
    for part in PART_NAMES:
        ...
        for group in GROUP_NAMES:
            ...
            ratio = safe_ratio(fitted_stats["mean"], reference_stats["mean"]) if fitted_stats["mean"] is not None else None
            part_result[group] = {
                ...
                "fitted_change_percent": (100.0 * (ratio - 1.0)) if ratio is not None else None,
            }
            if mask.any():
                collector[part][group]["reference"].append(reference_mag[mask].astype(np.float32).reshape(-1))
                collector[part][group]["fitted"].append(fitted_mag[mask].astype(np.float32).reshape(-1))
                collector[part][group]["error"].append(error_mag[mask].astype(np.float32).reshape(-1))
...
def aggregate_summary(rows, collector, skipped, device):
    ...
    for part in PART_NAMES:
        ...
        for group in GROUP_NAMES:
            ...
            reference = np.concatenate(values["reference"]) if values["reference"] else np.zeros(0)
            fitted = np.concatenate(values["fitted"]) if values["fitted"] else np.zeros(0)
            ...
            clip_ratios = [
                ...
                for row in rows
                if row["parts"][part][group]["fitted_to_reference_mean_ratio"] is not None
            ]
            summary["parts"][part][group] = {
                ...
                "fitted_to_reference_mean_ratio": ratio,
                "fitted_change_percent": (100.0 * (ratio - 1.0)) if ratio is not None else None,
                "clip_ratio_median": float(np.median(clip_ratios)) if clip_ratios else None,
                "clip_ratio_p05": float(np.percentile(clip_ratios, 5.0)) if clip_ratios else None,
                "clip_ratio_p95": float(np.percentile(clip_ratios, 95.0)) if clip_ratios else None,
            }
    return summary
...
def plot_summary(path, rows, summary):
    ...
    reference = [summary["parts"][part]["all"]["reference"]["mean"] for part in PART_NAMES]
    fitted = [summary["parts"][part]["all"]["fitted"]["mean"] for part in PART_NAMES]
    axes[0].bar(x - 0.18, reference, width=0.36, color="#d69b42", label="Dense SOKE reference")
    ...
    gt_clip = np.asarray([row["parts"]["wholebody"]["all"]["reference"]["mean"] for row in rows])
    fit_clip = np.asarray([row["parts"]["wholebody"]["all"]["fitted"]["mean"] for row in rows])
    fractions = np.asarray([row["filled_fraction"] for row in rows])
    scatter = axes[1].scatter(gt_clip, fit_clip, c=fractions, cmap="viridis", s=18, alpha=0.75)
...
def main():
    ...
    for split, row in tqdm(candidates, desc="evaluate GUAVA jerk"):
        try:
            rows.append(evaluate_sequence(row, split, args, fk, device, wanted_curves, collector))
        except Exception as exc:
            warnings.warn(f"Skipping {split}/{row.get('name')}: {exc}", RuntimeWarning)
            skipped.append({"split": split, "name": row.get("name"), "error": f"{type(exc).__name__}: {exc}"})
    ...
    with (args.out_dir / "jerk_per_sequence.jsonl").open("w", encoding="utf-8") as handle:
        for row in rows:
