...
def gap_condition_features(observed, max_gap=256, envelope_power=1.0):
    ...
    if not np.isfinite(envelope_power) or envelope_power <= 0:
        raise ValueError(f"envelope_power must be finite and positive, got {envelope_power}")
...
def apply_bounded_correction(
    ...
    if envelope.shape != scaffold.shape[:2]:
        raise ValueError(f"Envelope shape {envelope.shape} does not match {scaffold.shape[:2]}")

    batch, frames = scaffold.shape[:2]
    scaffold_matrix = rotation_6d_to_matrix(scaffold[..., :246].reshape(batch, frames, 41, 6))
    raw_matrix = rotation_6d_to_matrix(raw_prediction[..., :246].reshape(batch, frames, 41, 6))
    ...
    taper = envelope.to(device=scaffold.device, dtype=scaffold.dtype).unsqueeze(-1).unsqueeze(-1)
    ...
    bounded_expression = (
        scaffold[..., 246:]
        + expression_delta
        * envelope.to(device=scaffold.device, dtype=scaffold.dtype).unsqueeze(-1)
        * float(strength)
    )
    bounded = torch.cat([bounded_rot6d, bounded_expression], dim=-1)
    if valid_mask is not None:
        bounded = torch.where(valid_mask.unsqueeze(-1), bounded, torch.zeros_like(bounded))
