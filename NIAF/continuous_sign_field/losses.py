...
def masked_expression_l1(pred, target, mask):
    return masked_mean(torch.abs(pred[..., COMPACT6D_EXPRESSION] - target[..., COMPACT6D_EXPRESSION]), mask)
...
def hand_l1(pred_parts, target_parts):
    ...
    return torch.stack(losses).mean() if losses else pred_parts["wholebody"].new_tensor(0.0)
...
def temporal_joint_match_loss(pred, target, lengths, order=1, hand_weight=5.0, body_count=12):
    losses = []
...
def temporal_joint_magnitude_loss(values, lengths, order=1, hand_weight=5.0, body_count=12):
    losses = []
    for idx, length in enumerate(lengths.detach().cpu().tolist()):
        length = int(length)
        if length <= int(order):
...
def temporal_parts_match_loss(
    ...
    body_count = int(body_count if body_count is not None else target_parts["body"].shape[2])
    losses = [
...
def temporal_parts_magnitude_loss(
    ...
    body_count = int(body_count if body_count is not None else pred_parts["body"].shape[2])
    losses = [
...
def fk_temporal_dynamics_losses(
    ...
    if not any(float(weights.get(key, 0.0)) > 0 for _name, _order, key in order_specs):
        return total, losses
    if fk is None or target_parts is None:
        raise ValueError("FK temporal dynamics losses require fk and cached target_parts")
    ...
    for name, order, weight_key in order_specs:
        ...
        losses[f"loss_fk_{name}"] = temporal_parts_match_loss(
            ...
            hand_weight=hand_weight,
            body_count=body_count,
            include_hand_parts=bool(weights.get("fk_temporal_include_hand_parts", True)),
        )
...
def fk_temporal_regularization_losses(
    ...
    if not any(float(weights.get(key, 0.0)) > 0 for _name, _order, key in order_specs):
        return total, losses
    if fk is None or target_parts is None:
        raise ValueError("FK temporal regularization losses require fk and cached target_parts")
...
def endpoint_losses(
    ...
    if weights.get("lambda_rot6d", 0.0) > 0:
        losses["loss_rot6d"] = masked_feature_l1(pred, target, mask, hand_weight=hand_weight)
    ...
    if needs_fk:
        ...
        target_flat = {key: value[mask].to(device=pred.device, dtype=pred.dtype) for key, value in target_parts.items()}
        body_count = int(target_parts["body"].shape[2])
        ...
        if weights.get("lambda_hand_relative", 0.0) > 0:
            ...
            total = total + float(weights["lambda_hand_relative"]) * losses[
                "loss_hand_relative"
            ]
        if any(weights.get(key, 0.0) > 0 for key in ("lambda_vel", "lambda_vel_hand", "lambda_acc", "lambda_path")):
            pred_padded = pred_parts
            target_padded = {key: value.to(device=pred.device, dtype=pred.dtype) for key, value in target_parts.items()}
            ...
            if weights.get("lambda_vel_hand", 0.0) > 0:
                ...
                total = total + float(weights["lambda_vel_hand"]) * losses["loss_vel_hand"]
