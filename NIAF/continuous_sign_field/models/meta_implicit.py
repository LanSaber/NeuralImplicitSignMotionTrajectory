...
def masked_std(values, mask, dim=1):
    ...
    var = ((values - mean) ** 2 * mask_f).sum(dim=dim) / mask_f.sum(dim=dim).clamp_min(1.0)
...
class ContextToCode(nn.Module):
    ...
    def forward(self, scaffold, mask, lengths, text_tokens=None, text_mask=None):
        if text_tokens is None:
            ...
            text_context = masked_mean(text_tokens, text_mask, dim=1).to(dtype=scaffold.dtype)
...
class MetaImplicitResidualField(nn.Module):
    ...
    def initial_code(self, scaffold, mask, lengths, text_tokens=None, text_mask=None):
        return self.context_to_code(scaffold, mask, lengths, text_tokens=text_tokens, text_mask=text_mask)

    def forward(self, tau, scaffold, code, mask=None, condition=None):
        if mask is None:
            mask = torch.ones(scaffold.shape[:2], dtype=torch.bool, device=scaffold.device)
        if tau.ndim == 2:
            tau = tau.unsqueeze(-1)
        ...
        if self.condition_dim:
            ...
            feature_parts.append(condition.to(device=scaffold.device, dtype=scaffold.dtype))
        elif condition is not None and condition.shape[-1] != 0:
            raise ValueError("This residual field was constructed without conditioning features")
        features = torch.cat(feature_parts, dim=-1)
        residual = self.residual_scale.to(dtype=scaffold.dtype) * self.out(self.out_norm(self.net(features)))
        return residual * mask.unsqueeze(-1).to(residual.dtype)

    def predict(self, tau, scaffold, code, mask=None, condition=None):
        return scaffold + self.forward(tau, scaffold, code, mask=mask, condition=condition)
