...
def configured_gap_buckets(cfg):
    ...
    for index, raw in enumerate(configured):
        ...
        if not name or name in names:
            raise ValueError(f"Gap bucket names must be non-empty and unique, got {name!r}")
        if minimum <= 0 or maximum < minimum:
            raise ValueError(f"Invalid synthetic gap bucket {name}: [{minimum}, {maximum}]")
        if not math.isfinite(weight) or weight <= 0:
            raise ValueError(f"Gap bucket {name} must have a finite positive weight")
        names.add(name)
        buckets.append(
            {"name": name, "min": minimum, "max": maximum, "weight": weight}
        )
    return buckets
...
def build_masked_guava_view(batch, cfg, step=0):
    ...
    self_cfg = dict(cfg.get("self_supervision", {}))
    enabled = bool(self_cfg.get("enabled", False)) and float(self_cfg.get("weight", 0.0)) > 0
    if not enabled:
