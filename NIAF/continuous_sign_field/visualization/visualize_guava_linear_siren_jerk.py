...
from flow.render import SoftwareMeshRenderer, apply_view_transform, smplx182_to_vertices
...
from NIAF.continuous_sign_field.visualization.visualize_guava_completion_compare import (
    BACKGROUND,
    FILLED_TEXT,
    HEADER,
    MISSING_TEXT,
...
from NIAF.oracle_smplx_field.models.baselines import InterpolationBaseline
from NIAF.oracle_smplx_field.models.siren import DirectSirenPoseField


DEFAULT_REFERENCE_ROOT = Path("/media/cvpr/haomian/data/SOKE/How2Sign")
DEFAULT_OUT_DIR = Path(
    "visualize/NIAF/continuous_sign_field/guava_linear_siren_jerk_compare"
)
...
def parse_args():
    ...
    parser.add_argument("--input", type=Path, nargs="+", required=True, help="Completed GUAVA NPZ files.")
    ...
    parser.add_argument("--fps", type=float, default=0.0, help="0 uses source-video FPS.")
    parser.add_argument("--width", type=int, default=288, help="Width of each of the four panels.")
    parser.add_argument("--height", type=int, default=320, help="Height of each mesh/video panel.")
    ...
    parser.add_argument("--refit", action="store_true", help="Ignore compatible cached fits.")
...
def fit_direct_siren(completion, args, device, name):
    ...
    x_observed = torch.from_numpy(observed_rot6d(completion)).to(device=device, dtype=torch.float32)
    ...
    for _step in range(steps):
        if batch_points < len(indices):
            local = torch.randperm(len(indices), generator=generator, device=device)[:batch_points]
        else:
...
def load_or_fit_methods(path, completion, args, device):
    ...
    if fit_path.is_file() and not args.refit:
        try:
            ...
            if cached == expected and linear_motion.shape == completion["motion"].shape and siren_motion.shape == completion["motion"].shape:
                return linear_motion, siren_motion, siren_info, fit_path, True
        except Exception as exc:
            warnings.warn(f"Ignoring incompatible fit cache {fit_path}: {exc}", RuntimeWarning)
...
def reference_motion(path, root, expected_shape):
    ...
    if not pose_dir.is_dir():
        warnings.warn(f"Dense SOKE reference is unavailable: {pose_dir}", RuntimeWarning)
        return None, ""
...
def wholebody_jerk_curves(reference, linear, siren, observed_mask, fps, fk, device, batch_size):
    motions = []
    labels = []
    ...
    wholebody = joint_parts_chunked(fk, combined, device=device, batch_size=batch_size)["wholebody"]
    curves = {}
    start = 0
...
def curve_limits(curves):
    finite = np.concatenate(
        [np.asarray(value)[np.isfinite(value) & (np.asarray(value) > 0)] for value in curves.values()]
    )
    if finite.size == 0:
        return 1.0, 10.0
...
def data_y(value, low, high, top, bottom):
    ...
    alpha = (math.log10(value) - math.log10(low)) / max(math.log10(high) - math.log10(low), 1.0e-8)
...
def draw_curve(draw, values, color, bounds, limits, width=2):
    ...
    for index, value in enumerate(values):
        if not np.isfinite(value) or value <= 0:
            previous = None
            continue
        point = (data_x(index, len(values), left, right), data_y(value, low, high, top, bottom))
...
def build_jerk_chart(curves, masks, width, height, sequence_name, stats):
    ...
    for start, end in contiguous_runs(masks["touches_filled"]):
        ...
        overlay_draw.rectangle((x0, top, max(x1, x0 + 1), bottom), fill=(70, 190, 115, 28))
    ...
    for fraction in (0.0, 0.25, 0.5, 0.75, 1.0):
        ...
        draw.text((x - 13, bottom + 6), f"{fraction:.2g}T", fill=(160, 165, 174), font=font(10))

    for key in ("reference", "linear", "siren", "guava"):
        if key in curves:
            draw_curve(draw, curves[key], CURVE_COLORS[key], bounds, limits, width=2 if key != "guava" else 3)
...
def normalize_vertex_pair(linear_vertices, siren_vertices, indices, target_height=2.0):
    ...
    if extent <= 1.0e-6:
        extent = float(np.max(selected_centered.max(axis=0) - selected_centered.min(axis=0)))
...
def prepare_vertex_pair(linear_motion, siren_motion, args):
    ...
    if args.full_body:
        ...
        if args.cut_upper_body_mesh:
            faces = upper_faces
    linear_vertices, siren_vertices = normalize_vertex_pair(linear_vertices, siren_vertices, indices)
...
def compose_frame(panels, chart, chart_bounds, completion, frame_index, sequence_name):
    ...
    canvas = np.empty((header_h + panel_h + chart_h + footer_h, total_w, 3), dtype=np.uint8)
    ...
    if observed:
        ...
        guava_status = f"DISCARDED | {(reason or 'confidence filter').replace('_', ' ')}"
    ...
    draw.text((total_w - 340, footer_y + 5), "gold = observed   green = discarded/filled", fill=SUBTEXT, font=font(10))
    draw_timeline(
        draw,
...
def render_one(path, args, fk, device):
    ...
    if start >= end:
        raise ValueError(f"Empty requested frame range [{start}, {end}) for {path} with T={total}")
    ...
    linear_vertices, siren_vertices, faces = prepare_vertex_pair(linear_motion, siren_motion, args)
    ...
    if reader is None:
        ...
        warnings.warn(f"Source video has {source_frames} frames but fitting has {total}", RuntimeWarning)
    fps = float(args.fps) if float(args.fps) > 0 else float(video_meta.get("fps", 20.0))
    if not np.isfinite(fps) or fps <= 0:
        fps = 20.0

    dense_reference, reference_path = reference_motion(path, args.reference_root, linear_motion.shape)
    ...
    preview_path = args.out_dir / "previews" / f"{path.stem}_frame{wanted_preview:04d}.png"
    ...
    try:
        for frame_index in tqdm(range(start, end), desc=f"render {out_path.name}"):
            ...
            if reader is not None:
                try:
                    ...
                    if frame_index == start:
                        warnings.warn(f"Could not decode source frame {frame_index}: {exc}", RuntimeWarning)
            observed = bool(completion["observed_mask"][frame_index])
            linear_frame = renderer.render(linear_vertices[frame_index], color=LINEAR_MESH)
            ...
            guava_frame = (
                ...
                if observed
                else discarded_panel(renderer.width, renderer.height, completion["reasons"][frame_index])
            )
            panels = [
                source_panel(raw_source, renderer.width, renderer.height, args.source_fit),
                guava_frame,
                linear_frame,
                siren_frame,
            ]
            composed = compose_frame(panels, chart, chart_bounds, completion, frame_index, path.stem)
    ...
    metadata = {
        ...
        "source_video": str(source_video.resolve()) if source_video is not None and source_video.is_file() else "",
        ...
        "panel_order": ["source_rgb", "guava_sparse_smplx", "linear_rot6d", "direct_siren_rot6d"],
        "mesh_render": {
    ...
    out_path.with_suffix(".json").write_text(json.dumps(metadata, indent=2, ensure_ascii=False), encoding="utf-8")
