...
from flow.render import SoftwareMeshRenderer
...
from NIAF.continuous_sign_field.visualization.visualize_guava_linear_siren_jerk import (
    add_chart_cursor,
    chart_coordinates,
    contiguous_runs,
    curve_limits,
...
DEFAULT_INPUT_DIR = Path(
    "visualize/NIAF/continuous_sign_field/guava_bounded_meta_pilot/fits"
)
DEFAULT_OUT_DIR = Path(
    "visualize/NIAF/continuous_sign_field/guava_bounded_meta_pilot/render_soft_recon"
)
DEFAULT_REFERENCE_ROOT = Path("/media/cvpr/haomian/data/SOKE/How2Sign")
DEFAULT_EVALUATION = Path(
    "visualize/NIAF/continuous_sign_field/guava_bounded_meta_pilot/evaluation_summary.json"
)
...
def load_fit(path, method):
    motion_key = "finetuned_motion" if method == "mask_finetuned" else f"{method}_motion"
    ...
    for key in ("observed_mask", "filled_mask", "envelope"):
        if result[key].shape != (total,):
            raise ValueError(f"{path}: {key} has shape {result[key].shape}, expected {(total,)}")
    ...
    exact_error = float(
        np.max(np.abs(result["meta_motion"][observed] - result["slerp_motion"][observed]))
    )
    if exact_error > 1.0e-6:
        raise ValueError(f"{path}: meta output changed an observed anchor by {exact_error:g}")
    result["observed_exact_max_abs"] = exact_error
    return result


def wholebody_jerk_curves(reference, slerp, meta, observed_mask, fps, fk, device, batch_size):
    motions = []
    labels = []
    ...
    wholebody = joint_parts_chunked(
        fk, combined, device=device, batch_size=batch_size
    )["wholebody"]
    curves = {}
    offset = 0
    for label, motion in zip(labels, motions):
        joints = wholebody[offset : offset + len(motion)]
        curves[label] = np.linalg.norm(third_difference(joints, fps), axis=-1).mean(axis=1)
...
def build_jerk_chart(curves, masks, width, height, stats, meta_label="Bounded meta"):
    ...
    for power in range(int(math.ceil(math.log10(low))), int(math.floor(math.log10(high))) + 1):
        value = 10.0**power
...
def compose_frame(panels, chart, chart_bounds, completion, fit, frame_index, sequence_name):
    header_h = 64
    footer_h = 42
...
def render_one(path, args, fk, device):
    ...
    if not source_completion.is_file():
        raise FileNotFoundError(f"{path}: missing source completion {source_completion}")
    ...
    if start >= end:
        raise ValueError(f"Empty frame range [{start}, {end}) for {path} with T={total}")
    ...
    if reader is None:
        warnings.warn(f"No source RGB video available for {sequence_name}", RuntimeWarning)
    elif source_frames and source_frames != total:
        warnings.warn(
            f"Source video has {source_frames} frames but fitting has {total}", RuntimeWarning
        )
    ...
    range_suffix = "" if start == 0 and end == total else f"_frames{start:04d}-{end - 1:04d}"
    method_tag = (
        ...
        if retained_only
            ...
            if guava_self
            else ("mask_finetuned" if args.method == "mask_finetuned" else "bounded_meta")
        )
    )
    if improved_c2 and not guava_self and not retained_only:
        method_tag = "c2_fk_meta"
    out_path = args.out_dir / f"{sequence_name}_rgb_guava_slerp_{method_tag}_jerk{range_suffix}.mp4"
    ...
    preview_path = args.out_dir / "previews" / f"{sequence_name}_frame{wanted_preview:04d}.png"
            ...
            if reader is not None:
                try:
                    ...
                    if frame_index == start:
                        warnings.warn(
                            f"Could not decode source frame {frame_index}: {exc}", RuntimeWarning
                        )
            observed = bool(completion["observed_mask"][frame_index])
