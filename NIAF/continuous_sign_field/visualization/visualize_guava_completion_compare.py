...
def parse_args():
    ...
    parser.add_argument(
        "--out_dir",
        type=Path,
        default=Path("visualize/NIAF/continuous_sign_field/guava_completion_compare"),
    )
    parser.add_argument(
        "--source_video",
        type=Path,
    ...
    parser.add_argument("--full_body", action="store_true", help="Render the full mesh instead of upper body.")
    parser.add_argument(
        "--view_transform",
        default="none",
        choices=["none", "how2sign_front", "rot_x_180", "rot_y_180", "rot_z_180", "flip_y", "flip_z"],
    )
    parser.add_argument("--start_frame", type=int, default=0, help="Inclusive native frame index.")
    parser.add_argument("--end_frame", type=int, default=0, help="Exclusive native frame index; 0 uses the end.")
...
def load_completion(path):
    with np.load(path, allow_pickle=False) as data:
        ...
        trace_path = scalar_string(data["source_frame_trace"]) if "source_frame_trace" in data.files else ""
        method = scalar_string(data["completion_method"]) if "completion_method" in data.files else "unknown"

    total = len(motion)
    if motion.ndim != 2 or motion.shape[1] != COMPACT_DIM:
        raise ValueError(f"{path}: motion must have shape [T,{COMPACT_DIM}], got {motion.shape}")
    for name, value in (
        ...
        if value.shape != (total,):
            raise ValueError(f"{path}: {name} must have shape [{total}], got {value.shape}")
    ...
    if trace_path:
        ...
        if resolved_trace.is_file():
            ...
            warnings.warn(f"Missing recorded frame trace: {resolved_trace}", RuntimeWarning)
    reasons = [""] * total
...
def discarded_panel(width, height, reason):
    ...
    draw.text(((width - (box[2] - box[0])) // 2, height // 2 - 12), label, fill=MISSING_TEXT, font=font)
    short_reason = reason.replace("_", " ")[:54] if reason else "discarded by confidence filter"
    box = draw.textbbox((0, 0), short_reason, font=font)
    draw.text(((width - (box[2] - box[0])) // 2, height // 2 + 8), short_reason, fill=SUBTEXT, font=font)
...
def compose_frame(
    ...
    for panel_index, panel in enumerate(panels):
        canvas[header_h : header_h + height, panel_index * width : (panel_index + 1) * width] = panel
    ...
    if observed:
        ...
        input_status = f"DISCARDED | {(reason or 'confidence filter').replace('_', ' ')}"
        fit_status = (
...
def render_one(path, args):
    ...
    try:
        for frame_index in tqdm(range(start, end), desc=f"render {out_path.name}"):
            ...
            writer.append_data(compose_frame(panels, completion, frame_index, path.stem))
    finally:
        writer.close()
        if reader is not None:
            reader.close()
    ...
    metadata = {
        ...
        "source_video": str(source_video.resolve()) if source_video is not None and source_video.exists() else "",
...
def main():
    ...
    if summary_path.is_file():
        try:
            ...
            warnings.warn(f"Could not read existing render summary: {exc}", RuntimeWarning)
    by_output = {
