...
from flow.smplx_features import COMPACT6D_DIM, COMPACT6D_EXPRESSION, rotation_6d_to_matrix
...
def split_rot6d_expr(x):
    if x.shape[-1] != COMPACT6D_DIM:
        raise ValueError(f"Expected compact rot6D dim {COMPACT6D_DIM}, got {tuple(x.shape)}")
