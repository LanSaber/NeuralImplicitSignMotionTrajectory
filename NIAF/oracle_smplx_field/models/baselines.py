...
from flow.smplx_features import matrix_to_rotation_6d, rotation_6d_to_matrix
from NIAF.oracle_smplx_field.geometry.rotation import EXPR_SLICE, NUM_ROTATIONS, ROT6D_SLICE
...
class SlerpBaseline:
    ...
    def fit(self, tau, x):
        ...
        mats = rotation_6d_to_matrix(
            torch.from_numpy(x_np[:, ROT6D_SLICE].astype(np.float32)).reshape(len(x_np), NUM_ROTATIONS, 6)
        )
    ...
    def predict(self, tau, device=None, dtype=torch.float32):
        ...
        rot6d = matrix_to_rotation_6d(torch.from_numpy(mats)).reshape(len(query), NUM_ROTATIONS * 6)
        ...
        out = np.concatenate([rot6d.detach().cpu().numpy(), expr], axis=-1).astype(np.float32)
        return torch.as_tensor(out, dtype=dtype, device=device)
