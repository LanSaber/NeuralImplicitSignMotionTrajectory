...
DEFAULT_CSL_ROOT = Path("/media/cvpr/haomian/data/SOKE/CSL-Daily")
DEFAULT_OUT_DIR = Path("/media/cvpr/haomian/data/SOKE_FLOW/csl_daily_upper_smplx")
...
class TorchlessUnpickler(pickle.Unpickler):
    """Load CSL-Daily annotation pickles without requiring torch."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepare CSL-Daily SMPL-X poses in SOKE_FLOW format."
    )
    parser.add_argument("--csl_root", type=Path, default=DEFAULT_CSL_ROOT)
...
def process_split(args, split, running_stats):
    ann_path = args.csl_root / f"csl_clean.{split}"
    if not ann_path.is_file():
        raise FileNotFoundError(f"Missing CSL-Daily annotation file: {ann_path}")
    ...
    for ann in annotations:
        ...
        if rel_path in seen_paths:
            raise RuntimeError(f"Duplicate output path in split {split}: {rel_path}")
        seen_paths.add(rel_path)

        tasks.append(
            {
                "name": name,
                "pose_dir": args.csl_root / "poses" / name,
                "out_path": out_path,
                "rel_path": rel_path,
    ...
    print(f"{split}: processing {len(tasks)} annotations with {workers} workers", flush=True)
