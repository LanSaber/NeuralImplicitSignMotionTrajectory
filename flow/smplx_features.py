...
def normalize_rotation_rep(rotation_rep):
    ...
    if value not in ROTATION_REPS:
        raise ValueError(f"Unsupported rotation representation {rotation_rep!r}; expected axis_angle or rot6d.")
    return value
...
def rotation_rep_stats_paths(data_dir, rotation_rep):
    ...
    return data_dir / "meta" / f"mean_{rotation_rep}.npy", data_dir / "meta" / f"std_{rotation_rep}.npy"
...
def axis_angle_to_matrix(axis_angle):
    ...
    if original_shape[-1] != 3:
        raise ValueError(f"Expected axis-angle with last dimension 3, got {tuple(original_shape)}")
...
def matrix_to_axis_angle(matrix):
    ...
    if original_shape[-2:] != (3, 3):
        raise ValueError(f"Expected rotation matrix with shape [..., 3, 3], got {tuple(original_shape)}")
...
def matrix_to_rotation_6d(matrix):
    ...
    if matrix.shape[-2:] != (3, 3):
        raise ValueError(f"Expected rotation matrix with shape [..., 3, 3], got {tuple(matrix.shape)}")
...
def rotation_6d_to_matrix(rotation_6d):
    ...
    if rotation_6d.shape[-1] != 6:
        raise ValueError(f"Expected 6D rotation with last dimension 6, got {tuple(rotation_6d.shape)}")
    a1 = rotation_6d[..., 0:3]
    a2 = rotation_6d[..., 3:6]
...
def compact_axis_angle_to_rot6d_torch(compact):
    ...
    if compact.shape[-1] != COMPACT_DIM:
        raise ValueError(f"Expected compact axis-angle last dimension {COMPACT_DIM}, got {tuple(compact.shape)}")
    return torch.cat(
        [
...
def compact_rot6d_to_axis_angle_torch(compact6d):
    ...
    if compact6d.shape[-1] != COMPACT6D_DIM:
        raise ValueError(f"Expected compact rot6d last dimension {COMPACT6D_DIM}, got {tuple(compact6d.shape)}")
    return torch.cat(
        [
...
def compact_rot6d_to_axis_angle(compact6d):
    ...
    with torch.no_grad():
        out = compact_rot6d_to_axis_angle_torch(torch.from_numpy(compact6d)).cpu().numpy()
    return out.astype(np.float32, copy=False)
...
def compact_from_smplx182(smplx):
    ...
    if smplx.ndim != 2 or smplx.shape[1] != FULL_SMPLX_DIM:
        raise ValueError(f"Expected SMPL-X array with shape [T, 182], got {smplx.shape}")
...
def smplx182_from_compact(compact):
    ...
    if compact.ndim != 2 or compact.shape[1] != COMPACT_DIM:
        raise ValueError(f"Expected compact array with shape [T, 133], got {compact.shape}")
...
def smplx182_from_representation(motion, rotation_rep=ROTATION_REP_AXIS_ANGLE):
    return smplx182_from_compact(compact_from_rotation_representation(motion, rotation_rep))
...
def extract_compact_from_pickle(path, key="smplx"):
    ...
    left_valid = to_numpy(data.get("left_valid", np.ones(len(compact))), dtype=np.float32).reshape(-1)
    right_valid = to_numpy(data.get("right_valid", np.ones(len(compact))), dtype=np.float32).reshape(-1)
...
def feature_weight_vector(hand_weight=3.0, device=None, rotation_rep=ROTATION_REP_AXIS_ANGLE):
    rotation_rep = normalize_rotation_rep(rotation_rep)
    weights = torch.ones(rotation_rep_dim(rotation_rep), dtype=torch.float32, device=device)
