...
DEFAULT_MODEL_DIR = Path(__file__).resolve().parents[1] / "deps" / "smpl_models"
...
def smplx182_to_vertices(smplx_params, model_dir=DEFAULT_MODEL_DIR, gender="NEUTRAL", device="cpu", batch_size=128):
    import smplx
    ...
    faces = np.asarray(get_layer(min(batch_size, len(smplx_params))).faces, dtype=np.int32)
    vertices = []
    torch.set_grad_enabled(False)
    for start in tqdm(range(0, len(smplx_params), batch_size), desc="SMPL-X", leave=False):
...
class SoftwareMeshRenderer:
    ...
    def render(self, vertices, color=(0.48, 0.78, 1.0, 1.0)):
        ...
        for face_idx in order:
            ...
            fill = tuple(np.clip(base_color * shade[face_idx], 0, 255).astype(np.uint8).tolist())
...
def add_label(frame, label, frame_idx, total_frames):
    ...
    draw.text((12, 26), f"{frame_idx + 1}/{total_frames}", fill=(185, 190, 198), font=font)
    return np.asarray(image)
...
def write_vertices_video(
    ...
    vertices = normalize_vertices(apply_view_transform(vertices, view_transform=view_transform))
    renderer = SoftwareMeshRenderer(faces, width=width, height=height, face_stride=face_stride)
